...
nsdm.flex3<-function(x=numeric(),
 ...
 # Check supplied model types
  ...
  mod_args_j@args$data<-lis$train[[x]][,-which(colnames(lis$train[[x]]) %in% c("Presence", "X", "Y", "level"))]
 ...
 if(mod_args_j@mod=="lgb.train"){
  ...
  if(mod_args_j@weight){
  mod_args_j@args$data<-lgb.Dataset(as.matrix(lis$train[[x]][,-which(colnames(lis$train[[x]])%in% c("Presence", "X", "Y", "level"))]), 
