...
#' @param model_name A character vector indicating the name(s) of the modelling algorithm(s) for which predicted values are provided
#'
#' @return A list of rasters filled with predicted values
#' @author Antoine Adde (aadde@unil.ch)
#' @export

nsdm.map<-function(template, nona_ix, pred, species_name, model_name, level=NA, nesting_name=NA, rcp_name=NA, period_name=NA){
...
names(pred_fit)<-gsub("_NA", "", paste(species_name, model_nameu, level, nesting_name, rcp_name, period_name, sep="_"))
