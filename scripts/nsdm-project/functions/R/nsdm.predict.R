...
ndata<-parallel::mclapply(splits_ix, function(x){data.frame(fit=predict(model, nwdata[x,], se.fit = FALSE, gc.level=1, type = "response", block.size=NULL))}, mc.cores = nsplits)
ndata_bck<-rbind.fill(ndata)
