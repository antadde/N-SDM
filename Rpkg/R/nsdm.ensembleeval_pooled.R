...
#'
#' @return A named list of ensemble evaluation scores for each replicate. A `pooled` attribute
#'   contains one evaluation per ensemble type based on pooled out-of-fold predictions.
#'
#'#'
#' @author Antoine Adde (antoine.adde@eawag.ch)
#' @export

nsdm.ensembleeval_pooled <- function(sets, level, model_names, species_name, nesting_name=NA, posthoc_nesting_name=NA, scratch_path){
...
scores_ensemble_pooled <- list()

# Helper: evaluate pooled out-of-fold predictions across CV replicates
pooled_eval <- function(preds, pa) {
  f_all <- unlist(preds, use.names = FALSE)
  pa_all <- unlist(pa, use.names = FALSE)

  keep <- is.finite(f_all) & !is.na(pa_all)
  nsdm.ceval2(
    f = f_all[keep],
    pa = pa_all[keep]
  )
...
for (z in seq_len(outerloop)) {
  ...
  score <- nsdm.ceval2(f = f,
                       pa = papa_glo[[z]])
  scores[[z]] <- score
  preds_oof[[z]] <- f
}
scores_ensemble[["GLO"]] <- simplify2array(scores)
scores_ensemble_pooled[["GLO"]] <- pooled_eval(preds_oof, papa_glo)
...
if ("covariate" %in% nesting_name) {
    ...
    scores_ensemble_pooled[["COV"]] <- pooled_eval(preds_oof, papa_reg)
  }
...
# REG without nesting
    ...
    scores_ensemble_pooled[["REG"]] <- pooled_eval(preds_oof, papa_reg)
...
target <- REG_posthoc_preds
	...
	pa_posthoc[[z]] <- papa_reg[[z]][good_ix]

if (any(c("multiply") %in% posthoc_nesting_name)){  
# Multiply
    f_m <- sqrt(rowMeans(as.data.frame(z_target), na.rm = TRUE) * z_glo_prob2)[[1]]
    score_m <- nsdm.ceval2(
    f = f_m,
    pa = papa_reg[[z]])
	scores_m[[z]] <- score_m
...
# Multiply weigthed
	f_mw <- (((rowMeans(as.data.frame(z_target), na.rm = TRUE) ^ w_reg) * (z_glo_prob2[[1]] ^ w_glo)) ^ (1 / w_sum))
...
# Average
   f_a <- ((rowMeans(as.data.frame(z_target), na.rm = TRUE) + z_glo_prob2[[1]]) / 2)
   score_a <- nsdm.ceval2(
   f = f_a,
   pa = papa_reg[[z]])
   scores_a[[z]] <- score_a
...
# Average weighted
   f_aw <- ((w_reg * rowMeans(as.data.frame(z_target), na.rm = TRUE)) + (w_glo * z_glo_prob2[[1]])) / (w_reg + w_glo)
...
if (any(c("posthoc") %in% nesting_name)) {
  if (exists("scores_m") && length(scores_m) > 0) {
    scores_ensemble[["MUL"]] <- simplify2array(scores_m)
    scores_ensemble_pooled[["MUL"]] <- pooled_eval(preds_m, pa_posthoc)
  }
  if (exists("scores_mw") && length(scores_mw) > 0) {
    scores_ensemble[["MULW"]] <- simplify2array(scores_mw)
    scores_ensemble_pooled[["MULW"]] <- pooled_eval(preds_mw, pa_posthoc)
  }
  if (exists("scores_a") && length(scores_a) > 0) {
    scores_ensemble[["AVG"]] <- simplify2array(scores_a)
    scores_ensemble_pooled[["AVG"]] <- pooled_eval(preds_a, pa_posthoc)
  }
  if (exists("scores_aw") && length(scores_aw) > 0) {
    scores_ensemble[["AVGW"]] <- simplify2array(scores_aw)
    scores_ensemble_pooled[["AVGW"]] <- pooled_eval(preds_aw, pa_posthoc)
  }
}

# Pooled out-of-fold evaluations
attr(scores_ensemble, "pooled") <- scores_ensemble_pooled
