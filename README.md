...
|:--------|:-------------|:------------------|
| `d3_evals` / `d11_evals-final` | Model evaluation metrics for individual algorithms and ensembles | `.rds` |
| `d4_varimps` | Covariate importance scores | `.rds` |
| `d10_nested-ensembles` | Final nested ensemble maps | `.tif` |
| `d16_ensembles-sce` | Final nested ensemble maps for each scenario × period combination | `.tif` |
| `sacct` | SLURM job summaries (status, memory, CPU usage) for monitoring and debugging | `.txt` |
